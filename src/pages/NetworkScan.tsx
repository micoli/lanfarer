...
import { ScanLine, StopCircle, Bookmark, BookmarkPlus, Check, X, AlertTriangle, Download } from "lucide-react";
import { exportCsv } from "../lib/exportCsv";
import { useDhcpConfig, useDhcpClients, useCreateDhcpClient, useIpCheck } from "../hooks/useBbox";
import type { DhcpClient } from "../lib/bbox/types";

interface PingStats { min: number; avg: number; max: number }
    ...
    return (
      <span className="flex items-center gap-1.5">
        ...
        <span className="text-xs font-mono text-slate-400 whitespace-nowrap">
          {host.pingStats.min.toFixed(1)}/{host.pingStats.avg.toFixed(1)}/{host.pingStats.max.toFixed(1)}
...
function PortsCell({ ports }: { ports?: number[] }) {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {ports.map((p) => <PortBadge key={p} port={p} />)}
    </div>
  );
}
...
function ScanHostRow({ host, isReserved }: { host: ScanHost; isReserved: boolean }) {
  ...
  const [draft, setDraft] = useState<Omit<DhcpClient, "id">>({
    enable: 1, hostname: "", macaddress: "", ipaddress: "", ip6address: "",
  });
  const create = useCreateDhcpClient();
  const { checking, conflict, clearConflict, check } = useIpCheck();

  function openAdd() {
    setDraft({ enable: 1, hostname: host.hostname ?? "", macaddress: host.mac ?? "", ipaddress: host.ip ?? "", ip6address: "" });
    setAdding(true);
  }
      ...
      <>
        <tr className="border-t border-slate-700/50 bg-slate-800/40">
          <td className="px-4 py-2" colSpan={2}>
            <input className="input-cell w-full" placeholder="Nom d'hôte" value={draft.hostname}
              onChange={(e) => setDraft({ ...draft, hostname: e.target.value })} />
          </td>
          <td className="px-4 py-2">
            <input className="input-cell font-mono w-full" placeholder="192.168.x.x" value={draft.ipaddress}
              onChange={(e) => { setDraft({ ...draft, ipaddress: e.target.value }); clearConflict(); }} />
          </td>
          <td className="px-4 py-2">
            <input className="input-cell font-mono w-full" placeholder="aa:bb:cc:dd:ee:ff" value={draft.macaddress}
        ...
        {conflict && (
          <tr className="bg-amber-500/10 border-t border-amber-500/20">
            <td colSpan={8} className="px-4 py-1.5 text-xs text-amber-400 flex items-center gap-1.5">
              <AlertTriangle size={12} />{conflict} — cliquer à nouveau pour forcer
            </td>
          </tr>
        )}
      </>
  ...
  return (
    <tr className="border-t border-slate-700/50 hover:bg-slate-800/40 transition-colors">
      ...
      <td className="px-4 py-2.5 font-mono text-xs text-slate-400 whitespace-nowrap">{host.mac || "—"}</td>
      ...
      <td className="px-4 py-2.5"><PingCell host={host} /></td>
      <td className="px-4 py-2.5 text-xs text-slate-400">{host.vendor || <span className="text-slate-600">—</span>}</td>
      <td className="px-4 py-2.5"><PortsCell ports={host.openPorts} /></td>
      <td className="px-4 py-2.5 text-xs text-slate-400 whitespace-nowrap">
        {host.smbName
          ? <><span className="text-slate-300">{host.smbName}</span>{host.smbDomain && <span className="text-slate-600"> / {host.smbDomain}</span>}</>
          : host.smbName === undefined ? <span className="text-slate-600">…</span>
      ...
      <td className="px-4 py-2.5">
        {isReserved ? (
          ...
          <button type="button" title="Ajouter une réservation DHCP" onClick={openAdd}
...
export default function NetworkScan() {
  ...
  useEffect(() => {
    if (subnet) return;
    const dhcp = Array.isArray(rawDhcp) ? rawDhcp[0]?.dhcp : (rawDhcp as { dhcp?: { minaddress?: string } })?.dhcp;
  ...
  const reservedMacs = useMemo<Set<string>>(() => {
    const clients = Array.isArray(rawClients)
      ...
      : [];
    return new Set(
      (clients as { macaddress?: string }[]).map((c) => c.macaddress?.toLowerCase() ?? "").filter(Boolean)
    );
  }, [rawClients]);
  ...
  return (
    <div className="p-6 flex flex-col gap-4 overflow-auto">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-lg font-semibold text-slate-100">Scan réseau</h1>
        <div className="flex items-center gap-2">
          <input type="text" placeholder="Sous-réseau (ex: 192.168.1.0/24)"
            value={subnet} onChange={(e) => setSubnet(e.target.value)} disabled={scanning}
          ...
          {hosts.length > 0 && (
            <button
              type="button"
              onClick={() => exportCsv("scan-reseau.csv",
                ["IPv4", "MAC", "Hostname", "mDNS", "Ping min", "Ping avg", "Ping max", "Vendor", "Ports", "SMB Name", "SMB Domain"],
                hosts.map((h) => [
                  h.ip, h.mac, h.hostname, h.mdnsName ?? "",
                  h.pingStats?.min ?? "", h.pingStats?.avg ?? "", h.pingStats?.max ?? "",
                  h.vendor, (h.openPorts ?? []).join(" "), h.smbName ?? "", h.smbDomain ?? "",
                ])
              )}
          ...
          {scanning ? (
            ...
            <button type="button" onClick={startScan}
              ...
              <ScanLine size={14} />Lancer le scan
            </button>
          )}
        </div>
      </div>
      ...
      {(scanning || progress.total > 0) && (
        <div className="flex flex-col gap-1.5">
          ...
          <span className="text-xs text-slate-500">
            {progress.done} / {progress.total} adresses scannées
            {hosts.length > 0 && ` — ${hosts.length} hôte${hosts.length > 1 ? "s" : ""} trouvé${hosts.length > 1 ? "s" : ""}`}
          </span>
        </div>
      )}
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="text-xs uppercase tracking-wider text-slate-500 border-b border-slate-700">
                <th className="px-4 py-3 font-medium whitespace-nowrap">IPv4</th>
                <th className="px-4 py-3 font-medium whitespace-nowrap">MAC</th>
                <th className="px-4 py-3 font-medium">Hostname / mDNS</th>
                <th className="px-4 py-3 font-medium whitespace-nowrap">Ping min/avg/max</th>
                <th className="px-4 py-3 font-medium">Vendor</th>
                <th className="px-4 py-3 font-medium">Ports</th>
                <th className="px-4 py-3 font-medium whitespace-nowrap">SMB</th>
      ...
      {!scanning && hosts.length === 0 && progress.total > 0 && (
        <p className="text-sm text-slate-500 text-center py-8">Aucun hôte trouvé</p>
      )}
    </div>
  );
}
